...
class UrlController extends Define{
    ...
    /*
    private function LoadClienteController(): void 
    {   
        $this->classLoad = "\\Sts\\Controllers\\ClienteController"; //ClienteController
        $classPage = new $this->classLoad;

        $metodo = $this->urlController;
        if(method_exists($classPage, $this->metodo)){
            $classPage->$metodo();
        }else{
            die("Erro: Por favor tente novamente. Caso o problema persista, entre em contato o administrador " . EMAILADM);
        }
